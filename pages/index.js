    ...
    <div style={{ display: 'flex', gap: 4 }}>
      {[1,2,3,4,5].map(n => (
        <button key={n} onClick={() => onChange && onChange(n)} style={{
          ...
          fontSize: 20, color: n <= value ? 'var(--accent)' : 'var(--border-hover)',
...
function BookCard({ book, onStartReading, onFinish, onDelete, isAdmin }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 'var(--radius)', padding: '1.5rem', marginBottom: 8,
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: book.why ? 12 : 0 }}>{book.author}</p>
...
function AddBookModal({ onAdd, onClose, password }) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Title" style={{
          width: '100%', marginBottom: 10, background: 'var(--bg-3)',
          border: '0.5px solid var(--border)', borderRadius: 6,
        ...
        <input value={author} onChange={e => setAuthor(e.target.value)} placeholder="Author" style={{
          width: '100%', marginBottom: 16, background: 'var(--bg-3)',
          border: '0.5px solid var(--border)', borderRadius: 6,
          padding: '10px 12px', fontSize: 14, color: 'var(--text)', outline: 'none',
        }} />
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
          ...
          <button onClick={onClose} style={{
            background: 'none', border: '0.5px solid var(--border)', borderRadius: 6,
            padding: '10px 20px', fontSize: 14, color: 'var(--text-muted)',
            cursor: 'pointer',
          }}>Cancel</button>
        </div>
      </div>
...
export default function Home() {
  ...
  async function handleDeleteRead(book) {
    await apiPost('delete_read', book)
  }

  async function handleRemoveReading(book) {
    await apiPost('remove_reading', book)
  }
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              {isAdmin && (
                <button onClick={() => setShowAddBook(true)} style={{
                  background: 'var(--accent)', border: 'none', borderRadius: 6,
                  padding: '5px 12px', fontSize: 12, fontWeight: 500, color: '#0a0a0a',
                  cursor: 'pointer',
                }}>+ Add book</button>
              )}
              {!isAdmin && (
        ...
        {tab === 2 && (
          <div>
            <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
              {[['newest', 'Newest'], ['oldest', 'Oldest'], ['rating', 'Top rated']].map(([val, label]) => (
                <button key={val} onClick={() => setSortBy(val)} style={{
                  ...
                  color: sortBy === val ? '#0a0a0a' : 'var(--text-muted)',
                  cursor: 'pointer',
                }}>{label}</button>
              ))}
            </div>
            ...
            {sortedRead.map((book, i) => (
              <BookCard key={i} book={book} isAdmin={isAdmin}
                onDelete={isAdmin ? handleDeleteRead : null} />
            ))}
          </div>
        )}
      </div>
